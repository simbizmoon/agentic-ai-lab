...
def create_stage9_openai_development_runtime(
    *,
    ...
    openai_client: Any,
    official_web_client: httpx.Client | None = None,
    ...
    official_web = Stage9VerifiedOfficialWebAcquisitionAdapter(
        provider=Stage9VerifiedOfficialSourceProvider(
            discovery=Stage9OpenAIOfficialSourceDiscovery(
                client=openai_client,
                model=planner_lock.model_name,
            ),
            reader=Stage9HttpExactOfficialDocumentReader(
                allowed_domains=("nist.gov",),
                client=official_web_client,
            ),
        )
    )
