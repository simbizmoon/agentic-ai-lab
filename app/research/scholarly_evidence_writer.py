"""Safely persist deterministic scholarly evidence artifacts."""
...
SCHOLARLY_EVIDENCE_DIRECTORY_MODE = 0o700
SCHOLARLY_EVIDENCE_FILE_MODE = 0o600


@dataclass(frozen=True)
class ScholarlyEvidenceArtifactPaths:
    """Paths created for one persisted scholarly evidence execution."""
...
class ScholarlyEvidenceWriteError(RuntimeError):
    """Raised when private scholarly artifacts cannot be persisted safely."""


class ScholarlyEvidenceWriter:
    """Persist one Step 5B result as private Markdown and JSON files."""

    def __init__(
        self,
        *,
        formatter: DeterministicScholarlyEvidenceFormatter | None = None,
    ) -> None:
        self._formatter = formatter or DeterministicScholarlyEvidenceFormatter()
    ...
    def write(
        ...
        *,
        ...
        try:
            execution_dir.mkdir(mode=SCHOLARLY_EVIDENCE_DIRECTORY_MODE)
            os.chmod(execution_dir, SCHOLARLY_EVIDENCE_DIRECTORY_MODE)
        except OSError as error:
            try:
                execution_dir.rmdir()
            except OSError:
                pass
            raise ScholarlyEvidenceWriteError(
                "scholarly evidence execution directory could not be created"
            ) from error
        ...
        return ScholarlyEvidenceArtifactPaths(
            execution_dir=execution_dir,
    ...
    def _write_artifacts(
        ...
        *,
        ...
        except (OSError, ScholarlyEvidenceWriteError) as error:
            self._cleanup_failed_write(
                execution_dir=execution_dir,
                temp_paths=temp_paths,
                installed_paths=installed_paths,
            )
            if isinstance(error, ScholarlyEvidenceWriteError):
                raise
            raise ScholarlyEvidenceWriteError(
                "scholarly evidence artifacts could not be written"
            ) from error
    ...
    def _prepare_temp_file(*, execution_dir: Path, target: Path, text: str) -> Path:
        ...
        try:
            with NamedTemporaryFile(
                ...
                os.fchmod(temp_file.fileno(), SCHOLARLY_EVIDENCE_FILE_MODE)
    ...
    def _validate_final_target(target: Path) -> None:
        if target.is_symlink() or target.exists():
            raise ScholarlyEvidenceWriteError(
                "scholarly evidence artifact target is unsafe"
            )
